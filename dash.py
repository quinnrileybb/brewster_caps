...
# in your app’s main code:
csv_path = download_csv_from_drive()
df = pd.read_csv(csv_path)

st.set_page_config(page_title="Cape 2025", layout="wide")
st.success("Cape Data 2025")
# …and the rest of your Streamlit app…
...
# ——— Apply count filter ———
        ...
        def get_category_data(data, category):
            if category == "Fastball":
                return data[(data["TaggedPitchType"].isin(["Four-Seam", "Sinker"])) | (data["RelSpeed"] > 85)]
            elif category == "93+":
...
# Add a dropdown filter by PitcherThrows (Combined, Left, Right)
        ...
        def get_category_data(data, category):
            if category == "Fastball":
                return data[(data["TaggedPitchType"].isin(["Four-Seam", "Sinker"]))]
            elif category == "93+":
                return data[data["RelSpeed"] >= 93]
            elif category == "High Spin":
                return data[data["SpinRate"] > 2400]
...
# Create a DataFrame to display the table
    ...
    # apply category filter
            if selected_category == "Fastball":
                df_pl = df_pl[(df_pl["TaggedPitchType"].isin(["Four-Seam","Sinker"])) | (df_pl["RelSpeed"] > 85)]
            elif selected_category == "Breaking Ball":
                df_pl = df_pl[df_pl["TaggedPitchType"].isin(["Slider","Curveball","Cutter"])]
            elif selected_category == "Offspeed":
                df_pl = df_pl[df_pl["TaggedPitchType"].isin(["Splitter","Changeup"])]
...
# Now compute the pitch usage metrics for each pitch type.
        pitch_usage_data = []
        for pt in sorted(df_player['TaggedPitchType'].unique(), key=lambda x: str(x)):
            df_pt = df_player[df_player["TaggedPitchType"] == pt]
            count = len(df_pt)
...
# Apply styling to the DataFrame.
    ...
    "LHH 2Strk%": "{:.1f}"
        ...
        overall_counts = df_player['TaggedPitchType'].value_counts()
        total_pitches = len(df_player)
        overall_usage = (overall_counts / total_pitches * 100).round(1)
        grouped = df_player.groupby("TaggedPitchType").agg({
            "RelSpeed": ["mean", "max"],
            "SpinRate": "mean",
            "Tilt_numeric": "mean",
        ...
        grouped = grouped.reset_index().rename(columns={"TaggedPitchType": "Pitch Type"})
...
# Build per-pitch-type rows
        results_list = []
        for pt in df_player_filtered["TaggedPitchType"].unique():
            df_pt = df_player_filtered[df_player_filtered["TaggedPitchType"] == pt]
            total_pt = len(df_pt)
            if total_pt == 0:
                continue
...
# Style & display
    ...
    # — Top-5 pitch types —
            top5 = df_event["TaggedPitchType"].value_counts().index.tolist()[:5]
    ...
    # — Render five small plots in their own columns —
            ...
            for i, col in enumerate(cols):
                with col:
                    if i < len(top5):
                        pt = top5[i]
                        subset = df_event[df_event["TaggedPitchType"] == pt]
    ...
    # Row 1: Movement Plot and Approach Angle Plot.
            ...
            with col1:
                ...
                for pitch_type in table1_df['Pitch Type']:
                    pitch_data = df_player[df_player['TaggedPitchType'] == pitch_type]
    ...
    # Row 2: Release Location Plot and Release Speed by Inning Plot.
            ...
            with col2:
                ...
                avg_release = df_player.groupby(['Inning', 'TaggedPitchType'])['RelSpeed'].mean().reset_index()
                plt.figure(figsize=(6,4))
                for pitch_type in avg_release['TaggedPitchType'].unique():
                    subset = avg_release[avg_release['TaggedPitchType'] == pitch_type]
            ...
            with col1:
                ...
                plt.figure(figsize=(6,4))
                sns.violinplot(x="RelSpeed", y="TaggedPitchType", data=df_player, palette="Reds", orient="h")
            ...
            df_player_sorted['PitchOrder'] = df_player_sorted.groupby(['GameID', 'TaggedPitchType']).cumcount() + 1
            ...
            avg_velo_by_group = df_player_sorted.groupby(['TaggedPitchType', 'PitchGroup'])['RelSpeed'].mean().reset_index()

    
            with col2:
                st.subheader("Velo by Pitch in Outing")
                plt.figure(figsize=(6, 4))
                for pitch_type in avg_velo_by_group['TaggedPitchType'].unique():
                    subset = avg_velo_by_group[avg_velo_by_group['TaggedPitchType'] == pitch_type]
            ...
            with col2:
                ...
                avg_velo_by_date = df_player_sorted_by_date.groupby(["TaggedPitchType", "Date"])["RelSpeed"].mean().reset_index()
                plt.figure(figsize=(6, 4))
                for pitch_type in avg_velo_by_date["TaggedPitchType"].unique():
                    subset = avg_velo_by_date[avg_velo_by_date["TaggedPitchType"] == pitch_type].sort_values("Date")
            ...
            with col1: 
                ...
                avg_ivb_by_date = df_player_sorted_by_date.groupby(["TaggedPitchType", "Date"])["InducedVertBreak"].mean().reset_index()
                plt.figure(figsize=(6, 4))
                for pitch_type in avg_ivb_by_date["TaggedPitchType"].unique():
                    subset = avg_ivb_by_date[avg_ivb_by_date["TaggedPitchType"] == pitch_type].sort_values("Date")
            ...
            with col1: 
                ...
                df_player['Date'] = pd.to_datetime(df_player['Date'], errors='coerce')
                df_player_sorted_by_date = df_player.sort_values("Date")
                avg_spin_by_date = df_player_sorted_by_date.groupby(["TaggedPitchType", "Date"])["SpinRate"].mean().reset_index()
                plt.figure(figsize=(6, 4))
                for pitch_type in avg_spin_by_date["TaggedPitchType"].unique():
                    subset = avg_spin_by_date[avg_spin_by_date["TaggedPitchType"] == pitch_type].sort_values("Date")
            ...
            with col2: 
                ...
                df_player_sorted['PitchOrder'] = df_player_sorted.groupby(['GameID', 'TaggedPitchType']).cumcount() + 1
                df_player_sorted['PitchGroup'] = np.ceil(df_player_sorted['PitchOrder'] / 3).fillna(0).astype(int)
                avg_spin_by_group = df_player_sorted.groupby(["TaggedPitchType", "PitchGroup"])["SpinRate"].mean().reset_index()
                plt.figure(figsize=(6, 4))
                for pitch_type in avg_spin_by_group["TaggedPitchType"].unique():
                    subset = avg_spin_by_group[avg_spin_by_group["TaggedPitchType"] == pitch_type]
        ...
        # #Pitch Analyzer Tab
            with tabs[3]:
                ...
                available_pitches = df_player["TaggedPitchType"].dropna().unique()
                selected_pitch_type = st.selectbox("Select Pitch Type", sorted(available_pitches))

                df_selected_pitch = df_player[df_player["TaggedPitchType"] == selected_pitch_type].copy()
